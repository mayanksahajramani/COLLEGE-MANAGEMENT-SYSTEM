...
public class AdminRepository implements AdminRepositoryImpl {
    ...
    public String deleteSingleManagement(int id) {
        ...
        try{
            Statement st= databaseConnection();
            int count = st.executeUpdate(deleteQuery+" where id='"+id+"'");
            if(count>0){
    ...
    private static final String deleteQuery_admin = "delete from admin";
    private static final String update_admin_set="update admin set ";
    private static final String readQuery_admin="select * from admin ";
    private static final String registerQuery_admin="insert into admin values('";
    private static Statement databaseConnectionAdmin(){
        Statement st = null;
        try{
            Class.forName("com.mysql.jdbc.Driver");
            String url="jdbc:mysql://localhost:3306/smartstart08";
    ...
    public String register(Admin admin) {
        ...
        try{
            Statement st = databaseConnection();
            int count = st.executeUpdate(registerQuery+admin.getId()+"','"+admin.getName()+"','"+admin.getEmail()+"')");
            if(count>0){
                result=registerMessage;
            }
        }catch(Exception e){
    ...
    public String updateSingleAdmin(Admin admins) {
        ...
        try {
            ...
            if (admins.getName()!=null&&admins.getEmail()!=null){
                ...
                if (admins.getName()!=null&&admins.getEmail()==null){
                    count=st.executeUpdate(update_management_set+"name='"+admins.getName()+where_id+admins.getId()+"'");
                }
                else {
                    count=st.executeUpdate(update_management_set+"email='"+admins.getEmail()+where_id+admins.getId()+"'");
                }
